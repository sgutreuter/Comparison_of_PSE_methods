#########################################################################
##       R PROGRAM: PSE_estimate_BMA-4.R
##
...
#########################################################################
## BMA loglinear model fitting to independent Mt capture
## probabilities
#########################################################################
## (t.start <- Sys.time())
## test <- bma.estimate(data = Mt.10K.025, events = 3:5,
...
print(t.start)
bma.Mt.10K.025 <- bma.estimate(data = Mt.10K.025, events = 3:5,
...
t.start <- Sys.time()
bma.Mt.10K.050 <- bma.estimate(data = Mt.10K.050, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
Sys.time()
...
t.start <- Sys.time()
bma.Mt.10K.100 <- bma.estimate(data = Mt.10K.100, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
Sys.time()
...
t.start <- Sys.time()
bma.Mt.10K.150 <- bma.estimate(data = Mt.10K.150, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
## Total elapsed time:
...
## Save the results
#########################################################################
save(bma.Mt.10K.025, bma.Mt.10K.050, bma.Mt.10K.100, bma.Mt.10K.150,
     file = file.path(datapath, "BMA_Mt_estimates.Rdata"))
cgwtools::lsdata(file.path(datapath, "BMA_Mt_estimates.Rdata"))
################################   END of FILE   ########################
