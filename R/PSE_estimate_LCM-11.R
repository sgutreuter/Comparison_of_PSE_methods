#########################################################################
##       R PROGRAM: PSE_estimate_LCM-11.R
##
...
(t.start <- Sys.time())
lcm.tdec.10K.100 <- lcm.estimate(data = tdec.10K.100, events = 2:5,
                               buffer_size = 1000000, thinning = 100,
                               burnin = 500000, samples = 50000,
...
## Save the results
#########################################################################
save(lcm.tdec.10K.100,
     file = file.path(datapath, "LCM_tdec_estimates-3.Rdata"))
################################   END of FILE   ########################
