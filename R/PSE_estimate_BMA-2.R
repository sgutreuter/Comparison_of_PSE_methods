#########################################################################
##       R PROGRAM: PSE_estimate_BMA-2.R
##
...
#########################################################################
## test <- bma.estimate(data = Mh.10K.025, events = 3:5,
...
print(t.start)
bma.Mh.10K.025 <- bma.estimate(data = Mh.10K.025, events = 3:5,
...
t.start <- Sys.time()
bma.Mh.10K.050 <- bma.estimate(data = Mh.10K.050, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
Sys.time()
...
t.start <- Sys.time()
bma.Mh.10K.100 <- bma.estimate(data = Mh.10K.100, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
Sys.time()
...
t.start <- Sys.time()
bma.Mh.10K.150 <- bma.estimate(data = Mh.10K.150, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
## Total elapsed time:
...
## Save the results
#########################################################################
save(bma.Mh.10K.025, bma.Mh.10K.050, bma.Mh.10K.100, bma.Mh.10K.150,
     file = file.path(datapath, "BMA_Mh_estimates.Rdata"))
cgwtools::lsdata(file.path(datapath, "BMA_Mh_estimates.Rdata"))
################################   END of FILE   ########################
