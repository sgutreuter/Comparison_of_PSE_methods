#########################################################################
##       R PROGRAM: PSE_estimate_LCM-5.R
##
...
seed_  <- 321
## startt <- Sys.time()
## test <- lcm.estimate(data = Mbh.10K.025, events = 2:5,
...
t.start <- Sys.time()
lcm.Mbh.10K.025 <- lcm.estimate(data = Mbh.10K.025, events = 2:5,
                               buffer_size = 1000000, thinning = 100,
                               burnin = 500000, samples = 50000,
...
t.start <- Sys.time()
lcm.Mbh.10K.050 <- lcm.estimate(data = Mbh.10K.050, events = 2:5,
                               buffer_size = 1000000, thinning = 100,
                               burnin = 500000, samples = 50000,
                               nreps = 400, a_alpha = 0.025,
                               b_alpha = 0.025, seed = seed_)
...
t.start <- Sys.time()
lcm.Mbh.10K.100 <- lcm.estimate(data = Mbh.10K.100, events = 2:5,
                               buffer_size = 1000000, thinning = 100,
                               burnin = 500000, samples = 50000,
                               nreps = 400, a_alpha = 0.025,
                               b_alpha = 0.025, seed = seed_)
...
t.start <- Sys.time()
lcm.Mbh.10K.150 <- lcm.estimate(data = Mbh.10K.150, events = 2:5,
                               buffer_size = 1000000, thinning = 100,
                               burnin = 500000, samples = 50000,
                               nreps = 400, a_alpha = 0.025,
                               b_alpha = 0.025, seed = seed_)
...
## Save the results
#########################################################################
save(lcm.Mbh.10K.025, lcm.Mbh.10K.050, lcm.Mbh.10K.100, lcm.Mbh.10K.150,
     file = file.path(datapath, "LCM_Mbh_estimates.Rdata"))
################################   END of FILE   ########################
