#########################################################################
##       R PROGRAM: PSE_estimate_BMA-5.R
##
...
#########################################################################
## BMA loglinear model fitting to independent Mbh capture
## probabilities
#########################################################################
## (t.start <- Sys.time())
## test <- bma.estimate(data = Mbh.10K.025, events = 3:5,
...
print(t.start)
bma.Mbh.10K.025 <- bma.estimate(data = Mbh.10K.025, events = 3:5,
...
t.start <- Sys.time()
bma.Mbh.10K.050 <- bma.estimate(data = Mbh.10K.050, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
Sys.time()
...
t.start <- Sys.time()
bma.Mbh.10K.100 <- bma.estimate(data = Mbh.10K.100, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
Sys.time()
...
t.start <- Sys.time()
bma.Mbh.10K.150 <- bma.estimate(data = Mbh.10K.150, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
## Total elapsed time:
...
## Save the results
#########################################################################
save(bma.Mbh.10K.025, bma.Mbh.10K.050, bma.Mbh.10K.100, bma.Mbh.10K.150,
     file = file.path(datapath, "BMA_Mbh_estimates.Rdata"))
cgwtools::lsdata(file.path(datapath, "BMA_Mbh_estimates.Rdata"))
################################   END of FILE   ########################
