#########################################################################
##       R PROGRAM: PSE_estimate_LCM-10.R
##
...
(t.start <- Sys.time())
lcm.tdec.10K.050 <- lcm.estimate(data = tdec.10K.050, events = 2:5,
                               buffer_size = 1000000, thinning = 100,
                               burnin = 500000, samples = 50000,
...
## Save the results
#########################################################################
save(lcm.tdec.10K.050,
     file = file.path(datapath, "LCM_tdec_estimates-2.Rdata"))
################################   END of FILE   ########################
