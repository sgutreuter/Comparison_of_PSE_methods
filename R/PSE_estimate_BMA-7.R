#########################################################################
##       R PROGRAM: PSE_estimate_BMA-7.R
##
...
#########################################################################
## BMA loglinear model fitting to independent Mbht capture
## probabilities
#########################################################################
## (t.start <- Sys.time())
## test <- bma.estimate(data = Mbht.10K.025, events = 3:5,
...
print(t.start)
bma.Mbht.10K.025 <- bma.estimate(data = Mbht.10K.025, events = 3:5,
...
t.start <- Sys.time()
bma.Mbht.10K.050 <- bma.estimate(data = Mbht.10K.050, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
Sys.time()
...
t.start <- Sys.time()
bma.Mbht.10K.100 <- bma.estimate(data = Mbht.10K.100, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
Sys.time()
...
t.start <- Sys.time()
bma.Mbht.10K.150 <- bma.estimate(data = Mbht.10K.150, events = 3:5,
                               priorNmax = 100000, nreps = 400,
                               progress = FALSE)
Sys.time() - t.start
## Total elapsed time:
...
## Save the results
#########################################################################
save(bma.Mbht.10K.025, bma.Mbht.10K.050, bma.Mbht.10K.100, bma.Mbht.10K.150,
     file = file.path(datapath, "BMA_Mbht_estimates.Rdata"))
cgwtools::lsdata(file.path(datapath, "BMA_Mbht_estimates.Rdata"))
################################   END of FILE   ########################
